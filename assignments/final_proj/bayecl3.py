...
def _(mo):
    ...
    \]
    This rule is "Bayesian" only in the sense that $m_1(t),m_2(t)$ come from the conjugate posteriors above — it never looks at how uncertain those means are. It is the natural point-estimate baseline against which to measure whether uncertainty is worth using at all.
    """)
    return
...
def _(mo):
    mo.md(r"""
    ### Decision rule 2 — Bayesian probability matching

    Use the whole posterior. Compute the current probability that patch 1 is actually better,
    \[
    q_1(t) = P(\mu_1>\mu_2 \mid \text{data so far}),
    \]
    by drawing many samples from each patch's marginal-$t$ posterior above and taking the empirical fraction with $\mu_1>\mu_2$ (this is a Monte Carlo shortcut for exactly the Behrens–Fisher mixture posterior derived analytically in chapter 2). Then allocate $p_1(t)$ as the nearest point on the allocation grid to $q_1(t)$.

    Unlike the greedy rule, $q_1(t)$ depends on the *spread* of both posteriors — on $n_1,n_2,\kappa_1,\kappa_2,S_1,S_2$, i.e. on exactly the information-share structure ($\gamma,\rho$) built up in chapter 2 — not only on the means. When the two posteriors are still wide (little data, or a patch with large estimated variance), $q_1(t)$ stays close to $0.5$ and the plant hedges; as evidence accumulates, $q_1(t)$ moves toward $0$ or $1$ and the plant commits.
    """)
    return
...
def _(np):
    ...
    def bf3_prob_mu1_greater_mu2(m1, n1, kappa1, S1, m2, n2, kappa2, S2, rng, n_mc=2000):
        """Monte Carlo P(mu1 > mu2 | data) via the marginal Student-t posteriors of each patch mean."""
        ...
        return float(np.mean(mu1_draws > mu2_draws))
    ...
    def bf3_decide_allocation(rule, m1, n1, kappa1, S1, m2, n2, kappa2, S2, grid, rng):
        if rule == "greedy":
            ...
            raw = bf3_prob_mu1_greater_mu2(m1, n1, kappa1, S1, m2, n2, kappa2, S2, rng)
        else:
...
def _(bf3_decide_allocation, bf3_update_patch, np):
    def bf3_simulate_run(
        ...
        for t in range(T):
            y1 = rng.normal(mu1_true, np.sqrt(sigma1_sq_true))
            y2 = rng.normal(mu2_true, np.sqrt(sigma2_sq_true))

            m1, n1, kappa1, S1 = bf3_update_patch(m1, n1, kappa1, S1, y1)
            m2, n2, kappa2, S2 = bf3_update_patch(m2, n2, kappa2, S2, y2)

            p1 = bf3_decide_allocation(rule, m1, n1, kappa1, S1, m2, n2, kappa2, S2, grid, rng)
            p2 = 1.0 - p1
...
def _(mo):
    ...
    Set the two patches' true means and variances, the prior, the horizon, the mass budget, the allocation grid, and the decision rule. Each run uses fresh random draws (change the seed to resample).
    """)
    return
...
def _(mo):
    ...
    bf3_grid_ui = mo.ui.slider(2, 21, step=1, value=11, label="allocation grid size")
    bf3_rule_ui = mo.ui.dropdown(options=["greedy", "probability_matching"], value="probability_matching", label="decision rule")
    bf3_seed_ui = mo.ui.number(0, 10_000, step=1, value=0, label="seed")
    mo.hstack([bf3_T_ui, bf3_dM_ui, bf3_grid_ui, bf3_rule_ui, bf3_seed_ui])
    return bf3_T_ui, bf3_dM_ui, bf3_grid_ui, bf3_rule_ui, bf3_seed_ui
...
def _(
    ...
    _res = bf3_simulate_run(
        ...
        rule=bf3_rule_ui.value, seed=bf3_seed_ui.value,
    )
    _t = range(1, bf3_T_ui.value + 1)

    _fig, (_ax1, _ax2) = plt.subplots(1, 2, figsize=(11, 4))
    _ax1.plot(_t, _res["p1"], color="navy", lw=1.8, drawstyle="steps-post")
    ...
    _ax1.set_title(f"allocation over time ({bf3_rule_ui.value})")
    _ax1.grid(alpha=0.25)

    _ax2.plot(_t, _res["cum_gain"], color="darkgreen", lw=2)
...
def _(mo):
    ...
    To say anything about the two decision rules, one run is not enough — everything above is a single noisy draw. Here we fix a small table of biologically named scenarios (equal patch means at a low/mid/high level, crossed with how the two patches' variances compare), run many replicate simulations of each scenario under both decision rules, and collect the results into a tidy dataframe.
    """)
    return
...
@app.cell
def _(pd):
    bf3_mean_levels = {"low": 1.0, "mid": 3.0, "high": 6.0}
    bf3_variance_pairs = {
        "equal": (1.0, 1.0),
        "patch1_more_variable": (4.0, 1.0),
        "patch2_more_variable": (1.0, 4.0),
    }

    bf3_scenario_rows = []
    for _mean_label, _mu in bf3_mean_levels.items():
        for _var_label, (_s1sq, _s2sq) in bf3_variance_pairs.items():
            bf3_scenario_rows.append({
                "scenario": f"mean={_mean_label}, var={_var_label}",
                "mean_level": _mean_label,
                "variance_pair": _var_label,
                "mu1": _mu, "mu2": _mu,
                "sigma1_sq": _s1sq, "sigma2_sq": _s2sq,
            })
    bf3_scenario_table = pd.DataFrame(bf3_scenario_rows)
    bf3_scenario_table
    return bf3_mean_levels, bf3_scenario_table, bf3_variance_pairs
...
def _(mo):
    ...
    bf3_grid_batch_ui = mo.ui.slider(2, 21, step=1, value=11, label="allocation grid size")
        ...
        for _rule in _rules:
            for _rep in range(bf3_N_ui.value):
                ...
                _summary_rows.append({
                    ...
                    "mean_level": _scenario["mean_level"],
                    "variance_pair": _scenario["variance_pair"],
                    "decision_rule": _rule,
                    "replicate": _rep,
...
def _(mo):
    mo.md(r"""
    ### Final gain by scenario and decision rule
    """)
    return


@app.cell
def _(bf3_scenario_table, bf3_summary_df, np, plt):
    _scenarios = list(bf3_scenario_table["scenario"])
    ...
    for _offset, _rule, _color in [(-1, "greedy", "coral"), (1, "probability_matching", "steelblue")]:
        _data = [
            bf3_summary_df.loc[
                (bf3_summary_df["scenario"] == _s) & (bf3_summary_df["decision_rule"] == _rule),
                "final_gain",
            ].values
            for _s in _scenarios
        ]
        _positions = np.arange(len(_scenarios)) + _offset * _width / 2
    ...
    _ax.set_xticks(np.arange(len(_scenarios)))
    _ax.set_xticklabels(_scenarios, rotation=30, ha="right", fontsize=8)
    _ax.set_ylabel("final gain $G(T)$")
    _ax.set_title("final gain across replicates: greedy (coral) vs. probability matching (blue)")
...
def _(mo):
    ...
    Mean allocation to patch 1 over time (± 1 s.d. across replicates), one panel per mean level × variance pair.
    """)
    return


@app.cell
def _(bf3_mean_levels, bf3_traj_df, bf3_variance_pairs, plt):
    _mean_labels = list(bf3_mean_levels.keys())
    _var_labels = list(bf3_variance_pairs.keys())

    _fig, _axes = plt.subplots(len(_mean_labels), len(_var_labels), figsize=(12, 8), sharex=True, sharey=True)

    for _i, _mean_label in enumerate(_mean_labels):
        for _j, _var_label in enumerate(_var_labels):
            _ax = _axes[_i, _j]
            for _rule, _color in [("greedy", "coral"), ("probability_matching", "steelblue")]:
                _sub = bf3_traj_df[
                    (bf3_traj_df["mean_level"] == _mean_label)
                    & (bf3_traj_df["variance_pair"] == _var_label)
                    & (bf3_traj_df["decision_rule"] == _rule)
                ]
            ...
            if _i == 0:
                _ax.set_title(f"var: {_var_label}", fontsize=9)
            if _j == 0:
                _ax.set_ylabel(f"mean: {_mean_label}\n$p_1(t)$", fontsize=9)
            if _i == len(_mean_labels) - 1:
                _ax.set_xlabel("step")
            _ax.grid(alpha=0.2)
