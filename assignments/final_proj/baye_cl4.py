...
def _(mo):
    ...
    \]
$\rho$ enters the inference only through how it splits the sampling uncertainty between the two patches — this "information share" is what the next section makes precise, and it is what a Bayesian decision rule in chapter 3 will actually have access to.
...
def _(bf_n1_ui, bf_n2_ui, np, plt):
    ...
    _rho_eq = _n1 / _n2  # ratio at which the two patches carry equal information (gamma = 1/2)
    ...
    _ax1.scatter([_rho_eq], [0.5], color="crimson", zorder=5)
    _ax1.set_title(r"Linear $\rho$")
    ...
    _ax2.axvline(_rho_eq, color="crimson", linestyle=":", lw=1.5)
    _ax2.scatter([_rho_eq], [0.5], color="crimson", zorder=5, label=rf"equal information: $\rho=n_1/n_2={_rho_eq:.2f}$")
    _ax2.set_xscale("log")
    _ax2.set_title(r"Logarithmic $\rho$")
...
def _(mo):
    mo.md(r"""
    $\gamma(\rho)$ is how much of the sampling uncertainty about $\bar y_1-\bar y_2$ is attributable to patch 1: it is exactly the "information share" each patch contributes to what the plant can learn about the mean contrast. $\gamma=1/2$ at $\rho=n_1/n_2$ — the point where both patches are equally informative — and $\gamma$ saturates toward 0 or 1 as one patch's variance dominates the other's. This is the quantity that will do the work in chapter 3: a decision rule that actually uses the posterior (rather than only its mean) inherits this information-share structure directly.
    """)
    return
...
def _(mo):
    ...
    This is also where the analytic chapters of the project hand off to the numerical one that follows: turning a posterior over $\delta$ (or $v$) into an allocation strategy requires a decision rule linking belief to root-mass investment. Chapter 3 builds exactly that: sampling both patches at each discrete step, updating $\mu_1,\sigma_1^2,\mu_2,\sigma_2^2$ as here, and asking what it costs — in expected total nutrient gain — to make that allocation decision from the mean of the posterior alone versus from the whole posterior.
    """)
    return
...
def _(mo):
    mo.md(r"""
    # Chapter 3 — root allocation as sequential information foraging

    The first two chapters built the inference machinery: how a plant, sampling nutrient concentration from two patches, should update its belief about $\mu_1,\mu_2,\sigma_1^2,\sigma_2^2$, and what that belief implies about the mean contrast $\delta=\mu_1-\mu_2$. This chapter puts that machinery to work in a simulated allocation task, and asks a narrower, more concrete question than "risk sensitivity": *does allocating new root mass according to the whole posterior, rather than just its mean, actually change how much nutrient the plant collects?*

    The original inspiration for this chapter is
    > Zachary P. Kilpatrick, Jacob D. Davidson, Ahmed El Hady, Normative theory of patch foraging decisions, arXiv:2004.10671 (2020).

    which develops a normative, Bayesian account of when an animal should *leave* a patch, spending its limited *time* between patches. The root system modeled here faces a structurally similar problem, but the currency is different: instead of choosing when to leave a patch in time, the root chooses how to split a fixed *mass budget* between two patches at every step. Where Kilpatrick et al. track a scalar belief (an arrival rate $\lambda$) and threshold it to trigger a discrete leave/stay decision, our setting keeps the full two-patch, two-parameter ($\mu,\sigma^2$) posterior from chapters 1–2, and asks how continuously graded mass-allocation decisions should depend on it — but the underlying idealization (non-depleting patches, a forager that only knows what it has sampled so far, a decision rule read off a posterior belief) is the same in spirit.
    """)
    return
...
def _(mo):
    ...
    Stated explicitly, since every one of them is a simplification the reader should be able to question:

    1. **Non-depleting patches.** Sampling a patch does not reduce its future yield — an idealization Kilpatrick et al. also use for patches large or slowly consumed relative to the forager's harvest rate. Each patch's true $(\mu_i,\sigma_i^2)$ is fixed for the whole simulation.
    2. **A fixed, known horizon $T$.** We interpret $T$ as the number of discrete sampling/growth steps before flowering — a fixed developmental deadline the plant can treat as known in advance, after which accumulated nutrient maps onto reproductive fitness. Everything here is about maximizing gain by that deadline, not about steady-state reward rate.
    3. **A constant mass budget $\Delta M$ per step.** New root mass grown at each step does not depend on the plant's accumulated energy so far. This isolates the allocation *decision* from a full growth-dynamics model (see the discussion in the previous turn); making $\Delta M$ energy-dependent is a natural but separate extension.
    4. **A finite menu of allocation splits.** Rather than a continuous $p_1\in(0,1)$, the plant chooses among a finite grid of allocation fractions (e.g. $0,0.1,0.2,\ldots,1$). This bounds how finely the plant can hedge between patches and keeps the decision space explicit and small, matching the spirit of Kilpatrick et al.'s discretized/idealized limits.
    5. **Investment is irreversible.** Root mass, once grown into a patch, stays there and keeps drawing nutrient every subsequent step; it cannot be moved to the other patch later. Only the new increment $\Delta M$ is up for allocation at each step.
    6. **Sampling is decoupled from investment.** Both patches are sampled once per step regardless of the current mass split — the belief-updating machinery does not depend on how much root mass is already there. The same sample used to update the belief is also the nutrient actually harvested that step, scaled by the mass currently invested in that patch.
    7. **Objective.** Maximize the total nutrient gain accumulated over the $T$ steps, $G(T)=\sum_{t=1}^T\big(M_1(t)y_1(t)+M_2(t)y_2(t)\big)$, where $M_i(t)$ is the cumulative mass invested in patch $i$ by step $t$.

    Assumptions 3 and 6 are exactly the "phase 1" simplifications flagged earlier: keeping the mass budget fixed and decoupling sampling from investment isolates one question — what the *decision rule* does with the posterior — from the growth-dynamics and exploration-cost questions that would come next.
    """)
    return
...
def _(mo):
    mo.md(r"""
    ### Belief updating: one conjugate update per patch, per step

    Each patch's belief about its own $(\mu_i,\sigma_i^2)$ is updated independently with the single-population conjugate machinery from chapter 1 — there is no need for the two-sample joint prior there, since the two patches generate independent data. Given a current posterior $(m,n,\kappa,S)$ for a patch and one new observation $y$, the sequential conjugate update (equivalent to re-deriving the batch update of chapter 1 with a single new point, $\bar y=y$ and zero sum of squares) is
    \[
    n'=n+1,\qquad m'=\frac{nm+y}{n'},\qquad \kappa'=\kappa+1,\qquad S'=S+\frac{n}{n'}(y-m)^2.
    \]
    Applying this once per step, per patch, is all the belief-updating step of the simulation needs to do. As in chapter 1 (Theorem 17.1), marginalizing $\sigma_i^2$ out gives a Student-$t$ marginal for the patch mean,
    \[
    \mu_i \mid \text{data} \;\sim\; m_i + \sqrt{\frac{S_i}{\kappa_i n_i}}\, T_i, \qquad T_i\sim t_{\kappa_i},
    \]
    which is exactly what the probability-matching rule below uses.
    """)
    return
...
def _(mo):
    mo.md(r"""
    ### Decision rule 1 — greedy plug-in

    Ignore everything about the posterior except its mean. At each step, invest the entire new increment $\Delta M$ in whichever patch currently has the higher posterior mean estimate:
    \[
    p_1(t) = \begin{cases}1 & m_1(t) > m_2(t) \\ 0 & m_1(t) < m_2(t)\\ 0.5 & m_1(t)=m_2(t)\end{cases}, \qquad p_2(t) = 1-p_1(t).
    \]
    This rule is "Bayesian" only in the sense that $m_1(t),m_2(t)$ come from the conjugate posteriors above — it never looks at how uncertain those means are. It serves purely as a baseline: the point against which to measure whether using the *whole* posterior (rule 2, below) actually changes anything.
    """)
    return
...
def _(mo):
    mo.md(r"""
    ### Decision rule 2 — Bayesian probability matching on the difference

    Greedy plug-in is included only as a baseline; the meaningful content of this chapter is here. It is reasonable to assume the plant senses *contrast* rather than two absolute levels — so treat the quantity of interest as the difference $\delta=\mu_1-\mu_2$ directly, exactly as chapters 1–2 defined it, and ask for its current posterior sign:
    \[
    q_1(t) = P(\delta>0 \mid \text{data so far}) = P(\mu_1>\mu_2\mid\text{data so far}).
    \]
    We get $p(\delta\mid\text{data})$ by drawing $\mu_1,\mu_2$ independently from each patch's marginal-$t$ posterior above and subtracting — this is the same Behrens–Fisher difference-of-means problem chapter 2 solved analytically, just resolved by direct posterior sampling rather than chapter 2's secondary prior on $\rho$. That extra machinery in chapter 2 exists specifically for the *frequentist* case, where only point estimates $s_1^2,s_2^2$ are available and $\rho$ must be treated as an unknown nuisance parameter with its own prior; here we already hold full posteriors for $\sigma_1^2,\sigma_2^2$, so sampling from them directly *is* the Bayesian answer, and chapter 2's $\gamma,\rho$ structure is exactly what governs how wide the resulting $\delta$-posterior is. Allocate $p_1(t)$ as the nearest point on the allocation grid to $q_1(t)$.

    $q_1(t)$ depends on the *spread* of both posteriors — on $n_1,n_2,\kappa_1,\kappa_2,S_1,S_2$ — not only on the means. When the two posteriors are still wide (little data, or a patch with large estimated variance), $q_1(t)$ stays close to $0.5$ and the plant hedges; as evidence accumulates, $q_1(t)$ moves toward $0$ or $1$ and the plant commits.
    """)
    return
...
@app.cell
def _(np):
    def bf3_update_patch(m, n, kappa, S, y):
        """One sequential conjugate Normal-Inverse-chi^2 update for a single new observation y."""
        n_new = n + 1.0
        m_new = (n * m + y) / n_new
        kappa_new = kappa + 1.0
        S_new = S + (n / n_new) * (y - m) ** 2
        return m_new, n_new, kappa_new, S_new


    def bf3_prob_delta_positive(m1, n1, kappa1, S1, m2, n2, kappa2, S2, rng, n_mc=2000):
        """Monte Carlo P(delta > 0 | data), delta = mu1 - mu2, via the marginal Student-t posteriors of each patch mean."""
        t1 = rng.standard_t(kappa1, size=n_mc)
        t2 = rng.standard_t(kappa2, size=n_mc)
        mu1_draws = m1 + np.sqrt(S1 / (kappa1 * n1)) * t1
        mu2_draws = m2 + np.sqrt(S2 / (kappa2 * n2)) * t2
        delta_draws = mu1_draws - mu2_draws
        return float(np.mean(delta_draws > 0))


    def bf3_snap_to_grid(value, grid):
        return grid[int(np.argmin(np.abs(grid - value)))]


    def bf3_decide_allocation(rule, m1, n1, kappa1, S1, m2, n2, kappa2, S2, grid, rng):
        if rule == "greedy":
            if m1 > m2:
                raw = 1.0
            elif m1 < m2:
                ...
                raw = 0.5
        elif rule == "probability_matching":
            raw = bf3_prob_delta_positive(m1, n1, kappa1, S1, m2, n2, kappa2, S2, rng)
        else:
            raise ValueError(f"unknown decision rule: {rule}")
        return bf3_snap_to_grid(raw, grid)
    return bf3_decide_allocation, bf3_snap_to_grid, bf3_update_patch


@app.cell
def _(bf3_decide_allocation, bf3_update_patch, np):
    def bf3_simulate_run(
        mu1_true, sigma1_sq_true, mu2_true, sigma2_sq_true,
        m0, n0, kappa0, S0,
        T, delta_M, grid_size, rule, seed,
    ):
        """Simulate one T-step run of the two-pot allocation task under a given decision rule.

        Two independent random streams are used: `data_rng` generates the environment
        (y1(t), y2(t)), and `decision_rng` drives the probability-matching Monte Carlo step.
        Keeping them separate means two calls with the same seed and the same true
        parameters but different decision rules see the *identical* sequence of samples,
        so the two rules can be compared side by side on the same realized environment.

        Returns a dict of per-step arrays: p1, gain, cum_gain, m1, m2 (posterior means).
        """
        data_rng = np.random.default_rng(seed)
        decision_rng = np.random.default_rng(seed + 1_000_000_007)
        grid = np.linspace(0.0, 1.0, grid_size)

        m1, n1, kappa1, S1 = m0, n0, kappa0, S0
        m2, n2, kappa2, S2 = m0, n0, kappa0, S0

        M1_cum, M2_cum = 0.0, 0.0
        cum_gain = 0.0

        p1_hist = np.zeros(T)
        gain_hist = np.zeros(T)
        cum_gain_hist = np.zeros(T)
        m1_hist = np.zeros(T)
        m2_hist = np.zeros(T)

        for t in range(T):
            y1 = data_rng.normal(mu1_true, np.sqrt(sigma1_sq_true))
            y2 = data_rng.normal(mu2_true, np.sqrt(sigma2_sq_true))

            m1, n1, kappa1, S1 = bf3_update_patch(m1, n1, kappa1, S1, y1)
            m2, n2, kappa2, S2 = bf3_update_patch(m2, n2, kappa2, S2, y2)

            p1 = bf3_decide_allocation(rule, m1, n1, kappa1, S1, m2, n2, kappa2, S2, grid, decision_rng)
            p2 = 1.0 - p1

            M1_cum += p1 * delta_M
            M2_cum += p2 * delta_M
            gain = M1_cum * y1 + M2_cum * y2
            cum_gain += gain

            p1_hist[t] = p1
            gain_hist[t] = gain
            cum_gain_hist[t] = cum_gain
            m1_hist[t] = m1
            m2_hist[t] = m2
        ...
        return {
            "p1": p1_hist,
            "gain": gain_hist,
            "cum_gain": cum_gain_hist,
            "m1": m1_hist,
            "m2": m2_hist,
        }
    return (bf3_simulate_run,)


@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""
    ### Play with a single simulation

    Set the two patches' true means and variances, the prior, the horizon, the mass budget, and the allocation grid. Both decision rules run on the *same* realized sequence of samples (same seed), so the two curves in each panel below are a direct, apples-to-apples comparison — not an artifact of different random draws.
    """)
    return
...
@app.cell
def _(mo):
    bf3_mu1_ui = mo.ui.slider(0.0, 10.0, step=0.1, value=3.0, label="μ₁ (true)")
    bf3_sigma1_ui = mo.ui.slider(0.1, 5.0, step=0.1, value=1.0, label="σ₁ (true)")
    bf3_mu2_ui = mo.ui.slider(0.0, 10.0, step=0.1, value=3.0, label="μ₂ (true)")
    bf3_sigma2_ui = mo.ui.slider(0.1, 5.0, step=0.1, value=1.0, label="σ₂ (true)")
    mo.vstack([
        mo.hstack([bf3_mu1_ui, bf3_sigma1_ui]),
        mo.hstack([bf3_mu2_ui, bf3_sigma2_ui]),
    ])
    return bf3_mu1_ui, bf3_mu2_ui, bf3_sigma1_ui, bf3_sigma2_ui


@app.cell
def _(mo):
    bf3_m0_ui = mo.ui.slider(0.0, 10.0, step=0.1, value=3.0, label="prior mean m₀")
    bf3_n0_ui = mo.ui.slider(0.1, 10.0, step=0.1, value=1.0, label="prior mean strength n₀")
    bf3_kappa0_ui = mo.ui.slider(0.5, 10.0, step=0.5, value=1.0, label="prior variance d.f. κ₀")
    bf3_S0_ui = mo.ui.slider(0.1, 20.0, step=0.1, value=1.0, label="prior variance scale S₀")
    mo.hstack([bf3_m0_ui, bf3_n0_ui, bf3_kappa0_ui, bf3_S0_ui])
    return bf3_S0_ui, bf3_kappa0_ui, bf3_m0_ui, bf3_n0_ui


@app.cell
def _(mo):
    bf3_T_ui = mo.ui.slider(5, 200, step=1, value=50, label="T (steps to flowering)")
    bf3_dM_ui = mo.ui.slider(0.1, 10.0, step=0.1, value=1.0, label="ΔM per step")
    bf3_grid_ui = mo.ui.slider(2, 21, step=1, value=11, label="allocation grid points (11 → steps of 0.1)")
    bf3_seed_ui = mo.ui.number(0, 10_000, step=1, value=0, label="seed")
    mo.hstack([bf3_T_ui, bf3_dM_ui, bf3_grid_ui, bf3_seed_ui])
    return bf3_T_ui, bf3_dM_ui, bf3_grid_ui, bf3_seed_ui
...
def _(
    ...
    _rule_colors = {"greedy": "coral", "probability_matching": "steelblue"}
    _results = {
        _rule: bf3_simulate_run(
            mu1_true=bf3_mu1_ui.value, sigma1_sq_true=bf3_sigma1_ui.value ** 2,
            mu2_true=bf3_mu2_ui.value, sigma2_sq_true=bf3_sigma2_ui.value ** 2,
            m0=bf3_m0_ui.value, n0=bf3_n0_ui.value, kappa0=bf3_kappa0_ui.value, S0=bf3_S0_ui.value,
            T=bf3_T_ui.value, delta_M=bf3_dM_ui.value, grid_size=bf3_grid_ui.value,
            rule=_rule, seed=bf3_seed_ui.value,
        )
        for _rule in ["greedy", "probability_matching"]
    }
    _t = range(1, bf3_T_ui.value + 1)

    _fig, (_ax1, _ax2) = plt.subplots(1, 2, figsize=(11, 4))
    for _rule, _res in _results.items():
        _ax1.plot(_t, _res["p1"], color=_rule_colors[_rule], lw=1.8, drawstyle="steps-post", label=_rule)
        _ax2.plot(_t, _res["cum_gain"], color=_rule_colors[_rule], lw=2, label=_rule)

    _ax1.axhline(0.5, color="gray", linestyle=":", lw=1)
    _ax1.set_ylim(-0.05, 1.05)
    _ax1.set_xlabel("step")
    _ax1.set_ylabel(r"allocation to patch 1, $p_1(t)$")
    _ax1.set_title("allocation over time")
    _ax1.legend(fontsize=9)
    _ax1.grid(alpha=0.25)

    _ax2.set_xlabel("step")
    _ax2.set_ylabel("cumulative gain $G(t)$")
    _ax2.set_title("cumulative nutrient gain")
    _ax2.legend(fontsize=9)
...
def _(mo):
    mo.md(r"""
    ### Batch simulation: many replicates across named scenarios

    To say anything about the two decision rules, one run is not enough — everything above is a single noisy draw. The scenario table below follows the design of

    > Anat Dener, ..., et al. (2016) — five treatments comparing patch mean and variability: three shared the same SD but increased mean (a "constant SD" family), and three shared the coefficient of variation (SD/mean) while both mean and SD increased together (a "constant CV" family).

    By convention **patch 1 is always the fixed, low reference patch** (low mean, low variance) — patch 2 is the one that changes across scenarios. This removes a redundancy: if the two patches share the same mean, it does not matter which one we call "more variable," so each mean level gets exactly one variance condition (not two mirrored ones), plus one added "variance only" scenario to keep the pure-information test from chapter 2 in the mix.
    """)
    return
...
@app.cell
def _(pd):
    bf3_mu1_ref, bf3_sd1_ref = 2.0, 1.0  # patch 1: fixed low reference (Lc, Lv)
    bf3_cv_ref = bf3_sd1_ref / bf3_mu1_ref

    bf3_mean_high_var_only = bf3_mu1_ref  # patch 2 mean, "variance only" scenario
    bf3_sd_high_var_only = 2.0            # patch 2 sd,   "variance only" scenario

    bf3_mean_mid, bf3_mean_high = 4.0, 8.0
    bf3_sd_mid_cv = bf3_cv_ref * bf3_mean_mid
    bf3_sd_high_cv = bf3_cv_ref * bf3_mean_high

    bf3_scenario_rows = [
        {"scenario": "control (equal)", "family": "same SD", "patch2_level": "low",
         "mu1": bf3_mu1_ref, "sigma1_sq": bf3_sd1_ref ** 2, "mu2": bf3_mu1_ref, "sigma2_sq": bf3_sd1_ref ** 2},
        {"scenario": "mean ↑ (mid)", "family": "same SD", "patch2_level": "mid",
         "mu1": bf3_mu1_ref, "sigma1_sq": bf3_sd1_ref ** 2, "mu2": bf3_mean_mid, "sigma2_sq": bf3_sd1_ref ** 2},
        {"scenario": "mean ↑ (high)", "family": "same SD", "patch2_level": "high",
         "mu1": bf3_mu1_ref, "sigma1_sq": bf3_sd1_ref ** 2, "mu2": bf3_mean_high, "sigma2_sq": bf3_sd1_ref ** 2},
        {"scenario": "variance ↑ only", "family": "variance/CV differs", "patch2_level": "low",
         "mu1": bf3_mu1_ref, "sigma1_sq": bf3_sd1_ref ** 2, "mu2": bf3_mean_high_var_only, "sigma2_sq": bf3_sd_high_var_only ** 2},
        {"scenario": "mean+var ↑ (mid, CV const)", "family": "variance/CV differs", "patch2_level": "mid",
         "mu1": bf3_mu1_ref, "sigma1_sq": bf3_sd1_ref ** 2, "mu2": bf3_mean_mid, "sigma2_sq": bf3_sd_mid_cv ** 2},
        {"scenario": "mean+var ↑ (high, CV const)", "family": "variance/CV differs", "patch2_level": "high",
         "mu1": bf3_mu1_ref, "sigma1_sq": bf3_sd1_ref ** 2, "mu2": bf3_mean_high, "sigma2_sq": bf3_sd_high_cv ** 2},
    ]
    bf3_scenario_table = pd.DataFrame(bf3_scenario_rows)
    bf3_scenario_table
    return (bf3_scenario_table,)


@app.cell
def _(mo):
    bf3_N_ui = mo.ui.slider(5, 200, step=5, value=30, label="replicates per scenario × rule")
    bf3_T_batch_ui = mo.ui.slider(5, 200, step=5, value=40, label="T (steps)")
    bf3_dM_batch_ui = mo.ui.slider(0.1, 10.0, step=0.1, value=1.0, label="ΔM per step")
    bf3_grid_batch_ui = mo.ui.slider(2, 21, step=1, value=11, label="allocation grid points")
    bf3_run_button = mo.ui.run_button(label="Run batch simulation")
    mo.vstack([
        mo.hstack([bf3_N_ui, bf3_T_batch_ui, bf3_dM_batch_ui, bf3_grid_batch_ui]),
        bf3_run_button,
    ])
    return (
        bf3_N_ui,
...
def _(
    ...
    mo.stop(not bf3_run_button.value, mo.md("*click the button above to run the batch simulation*"))

    _summary_rows = []
    _traj_rows = []
    _rules = ["greedy", "probability_matching"]
    _seed = 0

    for _, _scenario in bf3_scenario_table.iterrows():
        for _rule in _rules:
            for _rep in range(bf3_N_ui.value):
                _seed += 1
                _res = bf3_simulate_run(
                    mu1_true=_scenario["mu1"], sigma1_sq_true=_scenario["sigma1_sq"],
                    mu2_true=_scenario["mu2"], sigma2_sq_true=_scenario["sigma2_sq"],
                    m0=3.0, n0=1.0, kappa0=1.0, S0=1.0,
                    T=bf3_T_batch_ui.value, delta_M=bf3_dM_batch_ui.value,
                    grid_size=bf3_grid_batch_ui.value, rule=_rule, seed=_seed,
                )
                _summary_rows.append({
                    "scenario": _scenario["scenario"],
                    "family": _scenario["family"],
                    "patch2_level": _scenario["patch2_level"],
                    "decision_rule": _rule,
                    "replicate": _rep,
                    "final_gain": _res["cum_gain"][-1],
                })
                for _t_idx, _p1 in enumerate(_res["p1"]):
                    _traj_rows.append({
                        "scenario": _scenario["scenario"],
                        "family": _scenario["family"],
                        "patch2_level": _scenario["patch2_level"],
    ...
    bf3_summary_df = pd.DataFrame(_summary_rows)
    bf3_traj_df = pd.DataFrame(_traj_rows)
    mo.md(f"ran {len(bf3_summary_df)} simulations ({bf3_scenario_table.shape[0]} scenarios × {len(_rules)} rules × {bf3_N_ui.value} replicates).")
    return bf3_summary_df, bf3_traj_df


@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""
    ### Final gain by resource level

    One panel per patch-2 mean level, so the variance/CV effect and the greedy-vs-probability-matching effect are the visual focus rather than nine crowded scenario labels. The "low" panel compares the control (identical patches) against the pure variance-only case; the "mid"/"high" panels compare a same-SD mean increase against a same-CV mean-and-variance increase.
    """)
    return


@app.cell
def _(bf3_summary_df, np, plt):
    _levels = ["low", "mid", "high"]
    _families = sorted(bf3_summary_df["family"].unique())
    _fig, _axes = plt.subplots(1, len(_levels), figsize=(13, 4.5), sharey=True)

    for _k, _level in enumerate(_levels):
        _ax = _axes[_k]
        _sub_level = bf3_summary_df[bf3_summary_df["patch2_level"] == _level]
        _present_families = [f for f in _families if f in _sub_level["family"].unique()]
        _width = 0.35
        for _offset, _rule, _color in [(-1, "greedy", "coral"), (1, "probability_matching", "steelblue")]:
            _data = [
                _sub_level.loc[
                    (_sub_level["family"] == _fam) & (_sub_level["decision_rule"] == _rule),
                    "final_gain",
                ].values
                for _fam in _present_families
            ]
            _positions = np.arange(len(_present_families)) + _offset * _width / 2
            _bp = _ax.boxplot(_data, positions=_positions, widths=_width * 0.9, patch_artist=True, showfliers=False)
            for _box in _bp["boxes"]:
                _box.set_facecolor(_color)
                _box.set_alpha(0.6)
        _ax.set_xticks(np.arange(len(_present_families)))
        _ax.set_xticklabels(_present_families, rotation=15, ha="right", fontsize=8)
        _ax.set_title(f"patch 2 mean: {_level}", fontsize=10)
        _ax.grid(alpha=0.25, axis="y")

    _axes[0].set_ylabel("final gain $G(T)$")
    _fig.suptitle("final gain: greedy (coral) vs. probability matching (blue)")
    _fig.tight_layout()
    _fig
    return
...
def _(mo):
    mo.md(r"""
    ### Allocation trajectories by scenario

    Mean allocation to patch 1 over time (± 1 s.d. across replicates). Columns are patch 2's mean level; the top row is the "same SD" family, the bottom row is the "variance/CV differs" family (variance-only for the low column, CV-preserving for mid/high).
    """)
    return


@app.cell
def _(bf3_traj_df, plt):
    _levels = ["low", "mid", "high"]
    _families = ["same SD", "variance/CV differs"]

    _fig, _axes = plt.subplots(len(_families), len(_levels), figsize=(12, 6), sharex=True, sharey=True)

    for _i, _family in enumerate(_families):
        for _j, _level in enumerate(_levels):
            _ax = _axes[_i, _j]
            _panel = bf3_traj_df[(bf3_traj_df["family"] == _family) & (bf3_traj_df["patch2_level"] == _level)]
            if _panel.empty:
                _ax.axis("off")
                continue
            for _rule, _color in [("greedy", "coral"), ("probability_matching", "steelblue")]:
                _sub = _panel[_panel["decision_rule"] == _rule]
                _stats = _sub.groupby("t")["p1"].agg(["mean", "std"])
                _ax.plot(_stats.index, _stats["mean"], color=_color, lw=1.8, label=_rule)
                _ax.fill_between(
                    _stats.index,
            ...
            _ax.axhline(0.5, color="gray", linestyle=":", lw=1)
            _ax.set_ylim(-0.05, 1.05)
            if _i == 0:
                _ax.set_title(f"patch 2 mean: {_level}", fontsize=9)
            if _j == 0:
                _ax.set_ylabel(f"{_family}\n$p_1(t)$", fontsize=9)
            if _i == len(_families) - 1:
                _ax.set_xlabel("step")
            _ax.grid(alpha=0.2)

    _axes[0, 0].legend(fontsize=8, loc="lower right")
    _fig.suptitle("allocation to patch 1 over time, by scenario")
    _fig.tight_layout()
    _fig
    return
...
def _(mo):
    ...
    Under equal means and equal variances, neither rule has a real signal to act on — any daylight between the two rules' gain distributions there is pure noise, and is the right baseline against which to judge the asymmetric-variance scenarios. Under equal means but unequal variance, the probability-matching rule has genuine information to use (the less variable patch resolves its true mean faster) while the greedy rule can only chase noisy point estimates — this is the cleanest test of whether using the full posterior, rather than only its mean, pays off in this simplified model. As with chapter 2, none of this required a utility function, a threshold, or an appeal to risk sensitivity — only the ordinary Bayesian machinery built in chapters 1 and 2, applied one time step at a time.
    """)
    return
