...
def _(mo):
    ...
    mo.hstack([df1_f, df2_f])
    return df1_f, df2_f


@app.cell
def _(df1_f, df2_f, np, plt, sp):
    _x = np.linspace(0.01, sp.stats.f.ppf(0.99, df1_f.value, df2_f.value), 500)
    _fig, _ax = plt.subplots(figsize=(6, 3))
    _ax.plot(_x, sp.stats.f.pdf(_x, df1_f.value, df2_f.value), 'g-')
    _ax.set_title(f"F Distribution F({df1_f.value}, {df2_f.value})")
    _fig
    return
...
@app.cell
def _(df_t, np, plt, sp, spstats):
    _x = np.linspace(-5, 5, 500)
    _fig, _ax = plt.subplots(figsize=(6, 3))
    _ax.plot(_x, sp.stats.t.pdf(_x, df_t.value), 'm-', label=f"t ({df_t.value})")
    _ax.plot(_x, spstats.norm.pdf(_x, 0, 1), 'k--', alpha=0.5, label="N(0,1)")
...
def _(np, plt, sp):
    ...
    T = Z / np.sqrt(U / nu) # Theorem 17.1
    x = np.linspace(-5, 5, 500)
    plt.hist(Z, bins=100, density=True, color="blue", alpha=0.5,label="z", fill=False)
    plt.hist(U, bins=100, density=True, alpha=0.5,label="u", fill=False)
    plt.hist(T, bins=100, density=True, alpha=0.5,label="Simulation")
    plt.plot(x, sp.stats.t.pdf(x, df=nu), linewidth=2, label=f"Student t, df={nu}")
    plt.legend()
    plt.show()
    return
...
def _(np, plt, sp):
    ...
    for _s2 in _prior_sigma2: # Faint Normal populations sampled from PRIOR
        ...
        plt.plot(_x,_pdf,alpha=0.06)

    # Faint Normal populations sampled from POSTERIOR
    for _s2 in _post_sigma2:
        _pdf = sp.stats.norm.pdf(_x,loc=_mu,scale=np.sqrt(_s2))
        plt.plot(_x,_pdf,alpha=0.18)

    plt.plot(_x,_prior_pdf,linestyle=":",linewidth=3,label=(rf"Prior: "rf"$\sigma^2=S/\kappa={_sigma2_prior_est:.2f}$"))
    plt.plot(_x,_true_pdf,linewidth=3,label=(rf"True population: "rf"$N({_mu},{_sigma2_true})$, "rf"$\sigma^2={_sigma2_true}$"))
    plt.plot(_x,_post_pdf,linestyle="--",linewidth=3,label=(rf"Posterior: "rf"$\hat{{\sigma}}_B^2={_sigma2_post_est:.2f}$"))
    plt.scatter(_y,np.zeros_like(_y),marker="|",s=180,label=rf"Observed sample ($n={_n}$)")
    plt.xlabel(r"$y$")
    plt.ylabel("Density")
    plt.title(rf"Bayesian learning of $\sigma^2$ "rf"($n={_n}$, $K={_K}$ prior/posterior draws)")
    plt.legend()
    plt.grid(alpha=0.25)
...
def _(
    ...
    For $\mu_2$:
    \[
    {sym.latex(mu2_part_sym)}={sym.latex(mu2_completed_sym)}
    \]
    where
    \[
    ...
    For $\sigma^2$: $\kappa' = {sym.latex(kappa_post_sym)},\qquad S' = {sym.latex(S_post_sym)}.$ Therefore the three conjugate components are
    \[
    \mu_1 \mid \sigma^2, y \sim N(m_1', \sigma^2/n_1'), \qquad \mu_2 \mid \sigma^2, y \sim N(m_2', \sigma^2/n_2'), \Longrightarrow \sigma^2 \mid y \sim S' \cdot \mathrm{{Inv}}\chi^2_{{\kappa'}}.
    \]
    """)
    return
...
def _(mo):
    ...
    \[
    t="\"\"+ sym.latex(t_from_theorem_sym)+ r"\"\".
    \]
    ...
    \[
    "\"\"+ sym.latex(t_check_sym)+ r"\"\"=0.
    \]

    Thus, although the conditional posterior of \(\mu_d\) given \(\sigma^2\)is Normal, accounting for the uncertainty in the unknown common varianceproduces a Student-\(t\) marginal posterior for the difference of means.
    """)
    return
